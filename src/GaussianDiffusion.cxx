...
GenKokkos::GaussianDiffusion::GaussianDiffusion(const IDepo::pointer& depo, const GausDesc& time_desc,
                                                ...
                                                double nsigma, IRandom::pointer fluctuate, unsigned int weightstrat)
{
    ...
    // cout << "set_sampling() : npss=" << npss << ", ntss=" << ntss << ", m_deposition->charge() = " <<
    // m_deposition->charge() << ", raw_sum=" << raw_sum << endl;
    ...
    if (fluctuate) {
        ...
        for (size_t ip = 0; ip < npss; ++ip) {
            for (size_t it = 0; it < ntss; ++it) {
                ...
                float number =
                    fluctuate->binomial((int) (std::abs(m_deposition->charge())), oldval / m_deposition->charge());
...
// patch(row,col)
const GenKokkos::GaussianDiffusion::patch_t& GenKokkos::GaussianDiffusion::patch() const { return m_patch; }

const std::vector<double> GenKokkos::GaussianDiffusion::weights() const { return m_qweights; }
